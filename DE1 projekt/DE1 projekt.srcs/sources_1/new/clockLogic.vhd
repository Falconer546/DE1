...
architecture Behavioral of clockLogic is
    -- Èasové registry
    signal ms     : integer range 0 to 99 := 0;  -- setiny pro èasovaè, desetiny pro stopky
        ...
        if rising_edge(clk) then
            if rst = '1' then
                ...
                if mode = '0' then
                    ...
                    -- ÈASOVAÈ (setiny sekundy)
                    if timer_set = '0' then
                        for i in 9 downto 0 loop
                            if set_time(i) = '1' then
                                if i = 9 then
                                    -- SW(10): Speciální pøípad pro 10 minut
                                    min <= 9;
                                    sec <= 59;
                                    ms  <= 99;
                                else
