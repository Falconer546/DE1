...
entity clockLogic is
    port (
        ...
        time_zero : out std_logic
    );
end entity;

architecture Behavioral of clockLogic is
    -- ?asové registry
    signal ms     : integer range 0 to 99 := 0;  -- setiny pro ?asova?, desetiny pro stopky
        ...
        if rising_edge(clk) then
            if rst = '1' then
                ...
                if mode = '0' then
                    ...
                    -- ?ASOVA? (setiny sekundy)
                    if timer_set = '0' then
                        for i in 9 downto 0 loop
                            if set_time(i) = '1' then
                                if i = 9 then
                                    -- SW(10): Speciální p?ípad pro 10 minut
                                    min <= 9;
                                    sec <= 59;
                                    ms  <= 99;
                                else
                                    -- SW(1 až 9): klasické nastavení 1 až 9 minut
                                    min <= i + 1;
                                    sec <= 0;
                                    ms  <= 0;
                                end if;
            ...
            -- LED výstup pøi nule v èasovaèi
            if mode = '1' then
                if min = 0 and sec = 0 and ms = 0 then
                    time_zero <= '1';
                else
                    time_zero <= '0';
                end if;
    ...
    begin
        if mode = '1' then
            ...
            digits <= std_logic_vector(to_unsigned(min, 4)) &
                      ...
                      x"0" & x"0" & x"0";   -- zhasne display
        else
            -- stopky zobrazení ?íslic
            digits <= std_logic_vector(to_unsigned(min, 4)) &
                      std_logic_vector(to_unsigned(sec / 10, 4)) &
                      std_logic_vector(to_unsigned(sec mod 10, 4)) &
